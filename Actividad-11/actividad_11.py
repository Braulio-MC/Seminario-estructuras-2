...
class Tree:
    def __init__(self):
        self._root = None
        self._identifiers = []  # Lista que contiene los identificadores de los objetos en el arbol

    def _get_identifiers(self, root):  # Metodo que recopila los identificadores de los objetos en el arbol
        if root is None:
            return
    ...
    @staticmethod
    def _replace(old, new):  # Metodo que reemplaza los nodos y sus nodos padres cuando se eliminan (usado por remove)
    ...
    def _get_by_id(self, root, identifier):  # Metodo para obtener un nodo del arbol por su identificador
        if root is None:
            return None
        if identifier == root.identifier:
            return root
    ...
    def _exists(self, root, identifier):  # Metodo para saber si un nodo existe por su identificador
        if root is None:
            return False
        if identifier == root.identifier:
            return True
    ...
    @staticmethod
    def _breadth_first(root):  # Recorrido en anchura
        queue = Queue()
        if queue and root:
            queue.push(root)
    ...
    def _height(self, root):  # Metodo para obtener la altura del arbol
        if root is None:
            return 0
    ...
    def _get_the_lowest(self, tree):  # Metodo que retorna el nodo mas pequeño basandose en el identificador
        if tree is None or tree.prev_node is None:
            return tree
        return self._get_the_lowest(tree.prev_node)

    def _get_the_highest(self, tree):  # Metodo que retorna el nodo mas grande basandose en el identificador
    ...
    def empty(self):  # Metodo que comprueba si el arbol esta vacio
        return self._root is None

    def identifiers(self):  # Metodo para obtener e imprimir los identificadores
        self._identifiers = []
